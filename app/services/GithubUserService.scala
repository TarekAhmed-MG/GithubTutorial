...
import models.{APIError, FileCreateModel, FileCreateModelDTO, FileDeleteModel, FileDeleteModelDTO, FileModel, FileUpdateModel, FileUpdateModelDTO, RepositoriesModel, RepositoryModel, UserModel}
...
class GithubUserService @Inject()(connector:GithubUserConnector){
  ...
  def deleteGithubRepositoryFile(urlOverride: Option[String] = None,userName: String, repoName:String, path: Array[Byte], requestBody: FileDeleteModelDTO)(implicit ec: ExecutionContext): EitherT[Future, APIError, FileModel] =
    connector.delete[FileDeleteModel,FileModel](urlOverride.getOrElse(s"https://api.github.com/repos/$userName/$repoName/contents/${decoder(path)}"),requestBody.fileDeleteModel)
